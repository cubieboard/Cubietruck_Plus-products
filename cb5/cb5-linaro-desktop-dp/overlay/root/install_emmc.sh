...
LED2_TRIGGER="/sys/class/leds/green:pe04:led4/trigger"

LED1_BRIGHT="/sys/class/leds/blue:pd25:led1/brightness"
LED2_BRIGHT="/sys/class/leds/green:pe04:led4/brightness"
...
part_card()
{
	umount /dev/mmcblk1p*
	umount /dev/mmcblk0p*
	if [ -e /dev/${card}p5 ]; then
	sudo dd if=/dev/zero of=/dev/mmcblk1 bs=1M count=30  
	fi
fdisk /dev/$card <<EOF
o
n
...
install_emmc()
{
	...
	if [ -f ${INSTALL_EMMC_FILE} ]; then
		...
		sed -i '148s/-card2//' /mnt/emmc${BACK_EMMC_FILE} 
	...
	if [ -f ${BACK_EMMC_FILE} ]; then
		...
		sed -i '148s/u-boot-sun8iw6p1.bin/u-boot-sun8iws6p1-card2.bin/' /mnt/emmc${INSTALL_EMMC_FILE} 
        ...
        /bin/cubie-uboot-spl ${BOOT_FILE}/u-boot-spl-sun8iw6p1.bin ${BOOT_FILE}/sys_config.bin ${BOOT_FILE}/u-boot-spl-sun8iw6p1-with-sys-config.bin

        sudo dd if=${BOOT_FILE}/u-boot-spl-sun8iw6p1-with-sys-config.bin of=/dev/${card} bs=1024 seek=8
        if [ $? -ne 0 ]; then
                echo "err in dd u-boot-spl" >  ${ERROR_LOG}/error_log.txt
                led_when_err
        fi

        rm -f ${BOOT_FILE}/u-boot-spl-sun8iw6p1-with-sys-config.bin
        /bin/cubie-uboot ${BOOT_FILE}/u-boot-sun8iw6p1-card2.bin ${BOOT_FILE}/sys_config.bin ${BOOT_FILE}/u-boot-sun8iw6p1-with-sys-config.bin
        sudo dd if=${BOOT_FILE}/u-boot-sun8iw6p1-with-sys-config.bin  of=/dev/${card}   bs=1024 seek=19096
        if [ $? -ne 0 ]; then
                echo "err in dd u-boot-spl" >  ${ERROR_LOG}/error_log.txt
                led_when_err
        fi

        rm -f ${BOOT_FILE}/u-boot-sun8iw6p1-with-sys-config.bin
